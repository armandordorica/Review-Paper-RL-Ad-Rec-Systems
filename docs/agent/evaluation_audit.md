...
|---|---|---|---|---|
| `zhao2021dear` (DEAR / Douyin) | Offline off-policy training on logged sessions; evaluation in a **simulated online environment** (Algorithm 2 / Table 2). The AAAI paper does **not** report live A/B tests or production deployment. | Session accumulated reward $R=\sum r_t$; $r_t^{ad}$ = ad revenue, $r_t^{ex}$ = browse-continue signal; predicted-CTR features | Strong offline/simulated RL ad-policy example on real platform logs. Do **not** cite Table 2 as online A/B evidence. | **Verified (May 2026):** Experiments section states "simulated online environment." Review-paper claims of TikTok online A/B are unsupported by the primary source. |
| `zhao2020jointly` (Jointly Learning to Recommend and Advertise) | Offline off-policy training on TikTok logs; evaluation in a **simulated online environment** (same family as DEAR). No live A/B in the KDD paper. | Session dwell time, session length, session ad revenue; reward weights α, β | Joint ad+organic RL example on real logs with explicit rec/ads trade-off metrics. | **Verified (May 2026):** Experiments section states "simulated online environment." |
...
*   **Metrics Used:** Session accumulated reward $R=\sum r_t$ with $r_t=r_t^{ad}+\alpha r_t^{ex}$; $r_t^{ad}$ = ad-video revenue, $r_t^{ex}$ = browse-continue UX signal; ad features include predicted-CTR; sensitivity analysis reports $R^{ad}=\sum r_t^{ad}$ and $R^{ex}=\sum r_t^{ex}$.
*   **Reported Results:** In a **simulated online environment** built from Douyin short-video logs (Mar 2019), DEAR achieved session accumulated reward $R=10.96$ on held-out sessions, outperforming HDQN (10.27), GRU (9.87), DeepFM (9.23), and Wide & Deep (9.12) ($p<0.01$). The AAAI paper does not report live A/B tests or production deployment metrics.
...
*   **Reported Results:** In a **simulated online environment** on TikTok logs (Mar 2019), RAM-l beat baselines: session dwell time +0.61%, session length +0.83%, session ad revenue +4.70% ($p<0.01$). No live A/B or deployment metrics in the KDD paper.
