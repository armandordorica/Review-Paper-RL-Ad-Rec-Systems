...
%considerations on choosing individual proxies 

While Tables \ref{tab:proxy-metrics-revenue}, \ref{tab:proxy-metrics-fatigue}, and \ref{tab:proxy-metrics-engagement} summarize a wide range of proxy metrics for revenue, ad fatigue, and engagement, selecting any individual proxy requires care. Many proxies are attractive precisely because they provide dense, behaviorally grounded feedback that is easy to log and optimize. Click-based signals, for example, are widely used because they are frequent, explicit, and highly responsive to ranking changes. However, their interpretation is not always stable. A high click-through rate may indicate relevance, but it may also reflect curiosity, ambiguity, or even misleading presentation—for example, when users do not immediately realize that an item is sponsored. Such effects can improve short-term metrics while eroding trust or satisfaction over longer horizons that are difficult to measure in short experiments. Even within the same signal family, semantics can vary: short clicks are often interpreted as implicit negative feedback, whereas longer clicks or post-click dwell can be more indicative of genuine value. Similar tensions arise for other objectives. Diversity-related signals, for instance, may be desirable when they reflect broadening of user interests over time, yet high click entropy within a session can also indicate confusion, low relevance, or friction. Taken individually, these proxies can be valuable signals; taken together, however, they reveal that reward design is not simply a matter of choosing “good” metrics, but of reconciling partially informative, context-dependent, and sometimes conflicting measurements of user value. This is precisely what motivates the use of composite utility functions that blend multiple proxy signals rather than relying on any single one in isolation.

One is reward density and timeliness: click-based or short-term consumption signals are attractive because they provide frequent feedback and support efficient learning, whereas return- or retention-based signals may better reflect long-term value but are much sparser and more delayed. A second consideration is heterogeneity across contexts. The same behavioral event may have different implications depending on the user segment, traffic surface, or intervention setting, so aggregate improvements can mask localized harms. A third consideration is that the meaning of a proxy is often not stable across content types and formats. For example, dwell time may indicate satisfaction for educational content but not necessarily for lightweight entertainment, and a watch event may have different utility for short-form video than for static image content. These considerations suggest that individual reward signals should be treated as context-dependent approximations rather than universally valid measures of user value.
