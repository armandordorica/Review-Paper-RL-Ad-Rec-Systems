...
% Interpretability
\textbf{Interpretability.} Actions with human-readable meaning support pre-launch review, but restricting the action space to such forms limits the behaviors a policy can express. Horizon \cite{gauci2018horizon} exposes high-level business levers to engineers, which enables tracing learned decisions back to upstream signals such as commercial intent scores. LinkedIn's shadow bid \cite{yan2020ads} is similarly interpretable because operators can read the exchange rate between engagement and revenue directly from the learned parameter. SlateQ \cite{ie2019slateq}, by contrast, exposes only a state-conditional slate of (ad, slot) pairs with no low-dimensional summary, so reviewers cannot characterize the policy's behavior by inspection and must defer to live A/B measurement \cite{dulac2021challenges, verma2018programmatically, glanois2024survey}. The trade-off is between the breadth of behaviors the policy can express and the ability of operators to vet it before deployment.
