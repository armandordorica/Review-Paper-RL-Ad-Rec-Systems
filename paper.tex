...
Rather than replacing supervised learning, RL can be viewed as embedding supervised prediction within a sequential control framework. Deep models remain central for representation learning and reward estimation, while RL augments them with the machinery required to optimize policy-dependent, long-term objectives. 
...
First, it proposes a unifying decision-theoretic taxonomy for monetization-aware recommender systems based on the core components of a Markov Decision Process: state, action, reward, and policy.
Second, it synthesizes more than 150 academic and industrial works, organizing them according to how they operationalize these components in practice.

% How the paper is structured 
The paper begins with a historical overview of recommendation systems with monetization components, tracing the evolution from rule-based and heuristic approaches to tree-based models and modern deep learning architectures. This progression highlights the limitations of short-horizon optimization and motivates reinforcement learning as a framework for modeling long-horizon, policy-dependent objectives.The survey then introduces the core components of a Markov Decision Process—state, action, reward, and policy—as a unifying taxonomy. Rather than organizing prior work chronologically or by algorithmic family, we structure the literature according to how systems define these components in practice. A minimal RL environment (FrozenLake) is used as a conceptual scaffold to introduce the abstraction before mapping it to large-scale monetization-aware recommender systems.
