...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{0.15\textwidth}}
\toprule
\textbf{Historical Track} & \textbf{Evolution} & \textbf{Remaining Limitation} & \textbf{RL Formulation Link} & \textbf{Representative Sources} \\
\midrule
\textbf{Pricing and objectives} & Online ads moved from impression-based pricing toward click-, action-, and utility-based objectives that more directly encode advertiser and platform outcomes. & Single business metrics can overemphasize immediate revenue, clicks, or conversions while leaving user experience, fatigue, and retention under-specified. & Maps to \textbf{reward design}: the reward function must encode trade-offs among advertiser value, platform revenue, engagement, and long-term user value. & \cite{hu2004performance,fain2006sponsored,engel2008incorporating,yan2020ads} \\
\textbf{Representation} & Systems moved from hand-crafted segments and tabular features toward learned embeddings and sequential representations of users, ads, items, and context. & Better representations improve prediction, but pointwise scores do not by themselves determine which policy should be used when actions change future user behavior. & Maps to \textbf{state and action spaces}: user, item, ad, context, and policy-control variables define what the agent observes and what it can change. & \cite{covington2016deep,cheng2016wide,vorotilov2023scaling,Zhou2019} \\
\textbf{Decision horizon} & Decision-making moved from static rules and supervised prediction toward A/B testing, bandits, offline policy evaluation, and eventually multi-step RL. & Short-horizon methods can adapt among candidate actions, but they remain limited when ad load, fatigue, retention, and future training data depend on earlier recommendations. & Maps to \textbf{policy learning and evaluation}: the learning framework determines whether the system optimizes immediate response, one-step exploration, or cumulative return under policy-induced dynamics. & \cite{li2010contextual,bietti2021contextual,swaminathan2015counterfactual,zhao2021dear,gauci2018horizon} \\
\bottomrule
\end{tabularx}
