...
While recent and highly cited work was prioritized, papers introducing novel methodologies were also included regardless of citation count. To contextualize the selected works, this survey reviews each component of the Markov Decision Process, including states, actions, rewards, and policies, in terms of how it is operationalized in industrial recommender systems with ads, along with a discussion of the limitations of those approaches. Drawing on existing methods and lessons from industry, the resulting framework is intended to inform practitioners exploring how RL can be applied in their organizations to develop ad policies that balance monetization with organic engagement. Table~\ref{tab:comparison} summarizes how each MDP component maps to a monetization-aware recommendation setting.

\begin{table*}[ht]
    \centering
    \small
    \begin{tabular}{|>{\centering\arraybackslash}p{2.5cm}|p{11cm}|}
        \hline
        MDP Component & Ad Recommendations \\ 
        \hline
        Environment & The users. \\ 
        \hline
        Goal & Maximize revenue without compromising user experience. \\ 
         \hline
          Episode & A time period (T), typically 2 weeks, corresponding to an A/B experiment where enough data is collected to evaluate policy performance across users. \\ 
        \hline
        Rewards & 
        ...
        \hline
    \end{tabular}
    \caption{Mapping of MDP Components to Large-Scale Recommendation Systems for Ad Policy Optimization}
...
The goal of joint ad and organic recommendation is to optimize engagement and revenue over time. Deep learning advanced this goal by improving the representation of users, ads, organic items, and their interactions. In particular, learned embeddings improved representation by encoding users, ads, organic items, and context in a shared feature space, while neural ranking models improved the mapping from those representations to immediate objectives such as click and conversion signals, along with other short-term engagement proxies \cite{covington2016deep,cheng2016wide,vorotilov2023scaling}. However, these advances did not change the underlying optimization structure when models were trained on pointwise labels from logged impressions. In that setting, each impression is treated as an independent example generated by the historical ranking policy, and the model learns to predict the immediate label attached to that example. Because training is anchored to isolated impressions rather than ordered trajectories, the model has limited ability to represent sequence-dependent effects. This improves short-term response prediction, but it weakly captures how the order, spacing, and accumulation of prior recommendations shape later outcomes. This limitation motivated sequential models, including Transformer-based architectures, which can encode user histories and changing intent from ordered interaction sequences \cite{vaswani2017attention,Kang2018,Zhou2019}. Even with better sequence representations, however, the model still does not by itself solve the policy-learning problem: the platform must choose actions, such as ad load, placement, reserve prices, or ranking policy, whose effects are observed through future user responses and future logged data \cite{swaminathan2015counterfactual,zou2019reinforcement}. 
