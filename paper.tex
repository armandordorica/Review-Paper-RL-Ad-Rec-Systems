...
\paragraph{Evaluating and improving utility function formulation in practice.}
Validating and improving a utility function involves two distinct problems. \textbf{Reward validation} consists of verifying that a given configuration of weights, signs, and functional form produces the intended outcomes, that is, whether $U(x)$ as specified actually tracks the platform objectives it is meant to represent. In practice, this is assessed through long-horizon A/B experiments that track top-line platform metrics such as daily, weekly, and monthly active users (DAU/WAU/MAU) for engagement and direct revenue for monetization \cite{hohnhold2015focusing, gauci2018horizon}. Long-term holdout groups, in which a subset of users is withheld from a new policy for an extended period, are also used to isolate delayed effects that would not be detectable within a standard two-week experiment window \cite{hohnhold2015focusing, mcdonald2023spotify}. For instance, ByteDance uses a multi-objective reward framework in which different reward functions are designed for organic content and ads, with the relative weights validated through live experimentation \cite{zhao2020jointly}.

\textbf{Reward formulation search} (finding a better-specified $U(x)$) is substantially harder, and should not be confused with RL policy improvement, which takes a fixed reward as given and searches for a better policy. Here the search is over the reward function itself: its component selection, coefficient magnitudes, signs, and functional form. The number of candidate configurations grows combinatorially with the number of parameters: each additional weight or proxy component multiplies the number of combinations that must be evaluated, making exhaustive A/B testing prohibitively expensive at scale \cite{gauci2018horizon}. This creates two compounding problems. First, the sample size required to detect meaningful differences across a large number of treatment arms grows with the number of comparisons, straining experimentation infrastructure. Second, A/B testing is inherently limited to the treatments that are explicitly tested \cite{swaminathan2015counterfactual, dudik2011doubly}; it provides no information about the performance of configurations that were never deployed. Offline replay addresses this second limitation by enabling counterfactual estimation: logged interaction data from randomized experiments is re-weighted using importance sampling to estimate how untested configurations would have performed under historical traffic \cite{levine2020offline, swaminathan2015counterfactual, dudik2011doubly}. This allows practitioners to evaluate a much larger set of candidate configurations cheaply before committing to a live experiment.

Offline RL extends this further by not only evaluating fixed configurations but actively searching for improved reward formulations within the space of possible configurations, using logged data as a surrogate for live interaction \cite{levine2020offline}. Rather than testing each configuration independently, offline RL methods learn a policy that maximizes the blended utility while staying close to the behavior policy that generated the data, thereby limiting the risk of out-of-distribution extrapolation \cite{levine2020offline, chen2022off}. This makes offline RL a natural fit for reward formulation search in large-scale systems, where the cost of live experimentation is high and the search space is too large for exhaustive enumeration.
