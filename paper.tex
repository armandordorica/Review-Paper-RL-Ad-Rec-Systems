...
%What is the problem 
Platforms such as Instagram, TikTok, and Pinterest develop and deploy large-scale content recommendation systems whose revenue models depend on jointly optimizing monetization and long-term engagement in interactive environments \cite{zhao2020jointly} \cite{danescu2010competing}. 
% What happens if problem isn’t solved
Failing to achieve an adequate balance can result in issues such as ad fatigue, where users become less responsive to ads due to overexposure, and user churn, where users discontinue using the platform \cite{sagtani2023quantifying} \cite{silberstein2023combating}.\\
...
Despite these structural challenges, industrial platforms have largely addressed this problem using supervised deep learning models. Over the past decade, deep learning has emerged as the dominant paradigm for large-scale recommendation systems. Production systems at major platforms typically rely on deep neural models trained using supervised objectives defined on short-term engagement signals such as clicks, dwell time, or conversions. By learning rich representations of users, items, and context from massive interaction logs, these models estimate conditional expectations of immediate outcomes and use these predictions to guide content selection, ranking, and monetization decisions. This paradigm has delivered substantial gains in personalization accuracy and short-term monetization efficiency, and remains central to modern industrial recommender systems.

% Structural limitations of Deep Learning to solve this problem 
Supervised deep learning models are trained on data generated by a historical logging policy. While neural networks remain powerful function approximators, optimizing supervised objectives on this logged data does not directly address the sequential and interactive nature of the system. In such settings, SL is structurally insufficient for two key reasons. First, SL ignores policy-induced distribution shifts. It optimizes conditional expectations of immediate outcomes (e.g., $\mathbb{E}[r_t \mid s_t, a_t]$) using data generated under a historical policy. This implicitly assumes that the training distribution is fixed. In interactive systems, however, the recommendation policy shapes user behavior, and user behavior in turn determines future training data. This violates the stationarity assumptions underlying standard supervised optimization. Second, SL lacks a principled mechanism for evaluating counterfactual trajectories.  SL relies exclusively on realized trajectories. Because user journeys are highly personalized and path-dependent, most possible interaction sequences are never observed in logged data. Long-term outcomes are therefore not fixed labels, but functions of the policy that generates future trajectories.
