...
To formalize multi-objective trade-offs such as balancing revenue and user satisfaction, utility functions have been employed in online advertising since the early 2000s \cite{engel2008incorporating}. Drawing on microeconomic utility theory, platforms formalized multi-objective trade-offs through scalar utility functions that encode multiple objectives—such as revenue, user satisfaction, and long-term platform health—into a single metric, enabling principled comparison and optimization \cite{engel2008incorporating}. When integrated into MAB frameworks, this unified reward signal guides exploration toward policies that balance short-term performance (e.g., clicks or conversions) with long-term considerations such as user trust and fatigue. 
...
Recommendation systems typically optimize short-term observable signals (e.g., clicks or engagement) that act as proxies for longer-term objectives such as long term revenue and engagement. While embedding-based models greatly improved the ability to encode non-linear representations of users, items, and their interactions, using these signals to optimize for pointwise independent interactions may lead to a suboptimal optimization over the length of the horizon of optimization. The true goal is to jointly optimize for sum of engagement and monetization rewards over the long term. 

The nature of the problem calls for sequential modelling. The ability to attend to any part of the input at any time allows Transformers to model long-range dependencies with far greater efficiency and accuracy than RNN-based architectures \cite{vaswani2017attention}. 
...
\setlength{\tabcolsep}{4pt} % separation between columns 
\begin{tabularx}{\linewidth}{p{0.13\linewidth} p{0.27\linewidth} p{0.54\linewidth}}
% the decimal points allow to control the size of the table, not of the text.
\toprule
{\bfseries Objective} & {\bfseries Proxy Metric} & {\bfseries Short Description} \\
\midrule
    \multirow{5}{*}{\textbf{Revenue}} 
