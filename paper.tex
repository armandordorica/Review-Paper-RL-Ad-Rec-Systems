...
To address the limitations of short-term optimization, many large-scale platforms have explored reinforcement learning (RL) as a framework for long-horizon decision making \cite{afsar2022reinforcement}. RL models recommendation as a sequential process in which system actions influence both immediate outcomes and future user behavior. This perspective allows platforms to explicitly reason about trade-offs between short-term monetization and long-term user value. Engagement rewards might include metrics such as time spent on the platform, content views, clicks, saves, shares, or likes \cite{wang2022surrogate}. Revenue rewards can be tied to ad-related metrics like ad impressions, ad clicks, conversions, or purchases resulting from ad exposure. 
These different rewards are often combined into a single utility function that reflects the platform's overall objectives \cite{zhao2020jointly} \cite{zou2019reinforcement} \cite{xu2023optimizing}.\\

A common formulation expresses the reward at time $t$ as a weighted combination of engagement and revenue components:

$$
r_t = \alpha \cdot \text{Engagement}_t + \beta \cdot \text{Revenue}_t,
$$ where the weights reflect the platform's strategic trade-offs between user experience and monetization. Variants of this linear or non-linear utility formulation appear across a wide range of industrial systems and serve as the basis for many RL-based ranking and ad-allocation policies. Throughout this survey, we use this utility-based perspective to compare how different systems define rewards, constraints, and long-term objectives.\\
...
In light of the challenges outlined above, this paper introduces a unifying framework to implement RL for large-scale recommendation systems, with a particular focus on systems where recommendation decisions have direct monetization implications, such as advertising, sponsored ranking, and marketplace platforms. Although previous surveys address RL in recommendation systems \cite{lin2023survey, afsar2022reinforcement}, they do not specifically explore the \textbf{joint optimization of ads and organic content.} This paper addresses this limitation by reviewing approaches that move beyond traditional, myopic ad selection (which can compromise long-term user retention) to analyze how RL agents can jointly weigh immediate revenue against long-term engagement.\\

This survey makes three main contributions.
First, it introduces a unifying decision-theoretic taxonomy for monetization-aware recommender systems based on the core components of a Markov Decision Process: state, action, reward, and policy.
Second, it synthesizes over 150 academic and industrial works, organizing them according to how they operationalize these components in practice.
Third, it identifies common design patterns and open challenges in long-horizon optimization, reward design, and large-scale policy learning.\\

The paper begins with a historical overview of ad selection and ranking systems, tracing the evolution from simple rule-based models and heuristic approaches to tree-based models and modern deep learning techniques. Each of these stages is explored in detail, highlighting their limitations and the motivations for adopting RL as the next paradigm in recommendation systems. Following this contextual history and motivation, the paper introduces key RL components such as the action space, state space, reward functions, and policies. Rather than organizing prior work chronologically or by specific algorithms, this survey adopts the core components of a Markov Decision Process (state, action, reward, and policy) as a unifying taxonomy. We use a minimal RL environment, the FrozenLake problem, as a conceptual scaffold to illustrate these components. The goal is not to teach reinforcement learning, but to provide a consistent abstraction that allows us to systematically compare how monetization-aware recommender systems operationalize each component in practice.

The remainder of the paper is organized according to this taxonomy. We first describe the evolution of monetization-aware ranking systems, and then survey prior work by examining how different approaches define the action space, state representation, reward structure, and policy learning mechanisms.
