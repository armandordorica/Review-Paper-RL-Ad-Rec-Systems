...
This subsection addresses a second design question in monetization-aware recommendation systems: what signals should the state representation include, and how should they be organized to support effective policy learning? We introduce four structural properties that published systems implicitly or explicitly satisfy when designing state representations for ad policies: Markovianity, expressiveness, stability under distribution shift, and learnability. As with the action-space taxonomy in Section~\ref{action_space_representation}, this framework is synthesized from recurring design considerations across industrial and academic systems rather than inherited from any single source. The subsection first defines the state formally, then analyzes each property with the trade-offs it implies, summarizes how representative systems operationalize each property, and closes with the concrete signal groups used in practice.

% Formal definition
In RL, a state $s \in \mathcal{S}$ is a representation of the information the agent needs to select an action that maximizes expected cumulative reward \cite{Sutton1998}. The state space $\mathcal{S}$ is the set of such representations the system can observe. In ad recommendation, state representations typically combine static user attributes, context features (time of day, device, surface), behavioral history (clicks, impressions, dwell time), and learned embeddings of users, items, and past interactions \cite{covington2016deep, Zhou2018, pancha2022pinnerformer}.

% Markovianity
\textbf{Markovianity.} Standard RL convergence guarantees assume the Markov property: conditional on the current state, the future is independent of the past \cite{Sutton1998, puterman2014markov}. When the state omits relevant history, value estimates become biased and learned policies degrade, because the agent attributes outcome variation to the wrong causes \cite{russell2016artificial, kaelbling1998planning}. Ad recommendation environments are strictly non-Markov: user intent, auction dynamics, and content inventory evolve on timescales that no fixed feature set can fully capture \cite{dulac2021challenges}. In large-scale systems, Markovianity cannot be tested directly; practitioners instead use indirect signals, such as the stability of off-policy value estimates as the history window is extended, or residual autocorrelation in prediction errors \cite{swaminathan2015counterfactual, dudik2011doubly}. The practical response is to extend the state with longer behavioral histories and richer context until downstream metrics (prediction accuracy, policy stability, estimator variance) stop improving \cite{pancha2022pinnerformer, Zhou2018}. The problem is therefore one of approximation, not satisfaction: the goal is a state representation for which the residual non-Markov effect is small relative to other sources of error.

% Expressiveness
\textbf{Expressiveness.} The state must carry enough information for the policy to distinguish between actions whose expected values differ. Industrial systems operationalize expressiveness through three signal groups that target different sources of variance in the outcome: slow-changing user attributes for between-user variance \cite{koren2009matrix, naumov2019deep}, contextual metadata for within-user between-session variance \cite{cheng2016wide}, and recent behavioral signals for within-session variance \cite{Zhou2018, pancha2022pinnerformer}. More expressive states improve discrimination between candidate ads but increase input dimensionality, which enlarges the sample complexity of the policy and raises the risk of overfitting to sparse regions of the state space. This trade-off is the primary constraint on learnability.

% Stability under distribution shift
\textbf{Stability under distribution shift.} User preferences, content inventory, and advertiser behavior drift on timescales from minutes to quarters \cite{gama2014survey, dulac2021challenges}. State representations that rely exclusively on stale offline embeddings lose fidelity to current behavior; those that rely exclusively on online signals are noisy and offer little regularization. Hybrid representations, such as the embedding tables used in Monolith \cite{liu2022monolith} and the long-horizon embeddings in PinnerFormer \cite{pancha2022pinnerformer}, combine offline components for slow-changing attributes with online components for recent behavior, and update each on its natural cadence: batch for the former, near-real-time for the latter. In these systems, the online component meets request-serving latency budgets because it is updated continuously from the event stream; its smaller size is a consequence of limiting it to recent events, not the primary cause of the latency improvement \cite{liu2022monolith}. The trade-off is between representational stability, which enables learning from long windows of logged data, and responsiveness to recent shifts in user or auction behavior.

% Learnability
\textbf{Learnability.} A state representation is only useful if the policy can be learned from it within the available data budget. Low-dimensional engineered features are sample-efficient and interpretable but constrain the policy class \cite{cheng2016wide}. High-dimensional learned embeddings are more expressive but require large interaction volumes and can overfit to sparse regions of the state space, particularly for cold-start users and long-tail ads \cite{naumov2019deep, covington2016deep}. Systems mitigate this trade-off by projecting heterogeneous signals into a shared dimension with learned linear layers, which avoids the noise and information loss introduced by zero-padding or truncation \cite{pancha2022pinnerformer, xia2023transact}. Cold-start cases, where behavioral signals are absent, are typically handled with population-level priors and onboarding signals rather than by modifying the state representation itself \cite{covington2016deep}.

% Synthesis table across systems
Table~\ref{tab:state-space-comparison} summarizes how four representative systems operationalize the four properties. No system satisfies all four cleanly; each accepts an explicit trade-off tied to its deployment constraints.
...
\toprule
\textbf{Property} & \textbf{DIN \cite{Zhou2018}} & \textbf{PinnerFormer \cite{pancha2022pinnerformer}} & \textbf{DEAR \cite{zhao2021dear}} & \textbf{Monolith \cite{liu2022monolith}} \\
\midrule
Markovianity approximation & Attention over recent user-behavior sequence & Transformer over long user-action history & GRU-encoded action history feeding dual Q-networks & Hybrid embedding table updated per event \\
Expressiveness & User profile, target ad, attention-weighted history & Dense long-range sequence embedding per user & User, ad, and context features for joint engagement/ad reward & Collision-free embedding table for high-cardinality sparse features \\
Stability under shift & Periodic offline retrain & Periodic retrain with recent-sequence freshness constraints & Offline retrain on logged sessions & Near-real-time parameter updates \\
Learnability constraint & Attention cost grows with history length & Long sequences increase training cost & Requires sufficient logged trajectories per user & Memory cost grows with feature cardinality \\
Primary trade-off & History depth vs.\ attention compute & Sequence freshness vs.\ retrain frequency & Representational complexity vs.\ sample efficiency & Memory footprint vs.\ responsiveness \\
\bottomrule
\end{tabularx}
\caption{Comparison of state-representation design choices across representative ad-policy systems. Each row corresponds to one of the four structural properties introduced in this subsection; each column describes how a published system operationalizes that property. The final row states the primary trade-off each system accepts as a consequence of its choices.}
\label{tab:state-space-comparison}
\end{table*}

% Operationalization via concrete signals
In practice, state representations in ad recommendation systems are assembled from four signal groups, each aligned with one or more of the properties above:

\begin{itemize}
    \item \textbf{User attributes.} Demographics, account metadata, and long-term preferences, typically encoded as offline embeddings updated in batch \cite{koren2009matrix, covington2016deep}.
    \item \textbf{Contextual metadata.} Time of day, device type, locale, and surface (feed, search, notifications), which explain within-user between-session variance \cite{cheng2016wide}.
    \item \textbf{Behavioral history.} Recent clicks, impressions, dwell-time buckets, and ad-exposure counters, encoded as raw features or online embeddings refreshed from the event stream \cite{Zhou2018, zhou2019deep}.
    \item \textbf{Interaction and sequence embeddings.} Learned representations of user-item interaction sequences, produced by attention networks or Transformers and capturing both short-term and long-range dependencies \cite{pancha2022pinnerformer, vaswani2017attention, kang2018sasrec}.
\end{itemize}

Taken together, the four properties and their trade-offs give practitioners a structured way to reason about which signals to include in the state and how frequently to update them. The next subsection turns to policy learning, which operates on the state representation to select actions.
