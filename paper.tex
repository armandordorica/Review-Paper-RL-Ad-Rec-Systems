...
Table~\ref{tab:evolution-tracks} summarizes three distinct but interrelated evolutionary tracks in ad optimization: pricing and objectives, representation, and decision horizon. These dimensions evolved somewhat in parallel rather than strictly sequentially, and modern systems typically combine advances from all three tracks. The table also previews how this history maps onto the RL formulation used in the rest of the paper: pricing and objectives motivate reward design, representation choices shape state and action spaces, and decision horizon determines whether supervised prediction, bandits, or full RL is the appropriate policy-learning framework. Evaluating these choices is difficult because the possible policies, state definitions, action discretizations, and reward functions create a large and often intractable search space. Although online A/B testing remains the gold standard for causal validation, testing every candidate live is prohibitively expensive and risky. Offline replay and related counterfactual methods are therefore used to narrow the set of promising candidates before A/B testing, balancing broad coverage of the design space with high-confidence online validation.
...
\textbf{Track 1: Pricing \& Objectives \(\rightarrow\) Reward Design}\\[0.3em]
...
\textbf{Track 2: Representation \(\rightarrow\) State and Action Spaces}\\[0.3em]
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.25\textwidth} >{\raggedright\arraybackslash}p{0.31\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth}}
\toprule
\textbf{Stage} & \textbf{Encoded Information} & \textbf{Limitation} & \textbf{When Sufficient} \\
\midrule
\textbf{Rules and segments} & Demographics, location, and hand-crafted eligibility rules & Coarse personalization and manual updates & Small inventories or strict policy constraints \\
\textbf{Tabular features} & Click history, session statistics, and engineered context & Limited nonlinear interaction modeling & Interpretable models or limited compute \\
...
\textbf{Sequential encoders} & User histories and evolving interests & More complex training and harder action attribution & Session-aware ranking and dynamic intent \\
\midrule
...
% Track 3: Decision Horizon
\textbf{Track 3: Decision Horizon \(\rightarrow\) Policy Learning and Evaluation}\\[0.3em]
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.25\textwidth} >{\raggedright\arraybackslash}p{0.31\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth}}
\toprule
...
\caption{Three Evolutionary Tracks in Ad Optimization and Their Link to the RL Formulation. Modern systems combine advances across pricing objectives, representation, and decision horizon. The tracks motivate the later discussion of reward design, state and action spaces, and policy learning.}
