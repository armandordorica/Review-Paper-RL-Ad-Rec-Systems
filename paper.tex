...
Combining ad and organic signals into a single utility is harder than combining signals within either content type alone, for two reasons that are specific to the joint optimization setting. First, the two content types operate on \textbf{different reward timescales}. Monetization outcomes such as clicks and conversions are attributed within a short window — commonly seven days in industry practice \cite{yan2020ads, zhang2018whole} — and therefore generate relatively dense, observable feedback. Organic engagement outcomes such as retention, habit formation, and long-term satisfaction manifest over much longer horizons, often measured across weeks or months \cite{hohnhold2015focusing, mcdonald2023spotify}. A simple additive utility collapses this temporal distinction into a single scalar, implicitly treating a same-day ad click as equivalent to a longer-term retention signal. This can cause the policy to over-weight near-term monetization, since its reward is denser and more immediately attributable than the delayed effects of organic content quality.

Second, the two objectives are expressed in \textbf{incommensurable units}. Ad utility is typically computed as $\text{bid} \times \text{pCTR}$, a monetary quantity \cite{yan2020ads, zhang2018whole}, whereas organic utility is expressed in behavioral units such as saves, shares, or dwell time \cite{wang2022surrogate, mcdonald2023spotify}. Combining them requires a conversion factor: the shadow bid $\alpha$ in LinkedIn's constrained optimization formulation \cite{yan2020ads} serves precisely this purpose, converting engagement utility into revenue-equivalent units so that the two objectives can be compared. However, this multiplier rests on the simplifying assumption that the exchange rate between engagement and revenue is time-invariant and context-agnostic. In practice, the marginal value of engagement relative to revenue varies across user segments, content types, and business priorities \cite{hohnhold2015focusing}, making a single static multiplier an approximation whose miscalibration can systematically skew the learned policy. Together, these two structural asymmetries explain why the canonical utility function below should be understood as a practical approximation of platform value rather than a precise formalization of it.
